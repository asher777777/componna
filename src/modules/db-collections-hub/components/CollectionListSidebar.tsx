...
const ICON_MAP: Record<string, React.ReactNode> = {
  ...
  UserCheck: <UserCheck className="w-4 h-4 text-purple-400" />,
...
export const CollectionListSidebar: React.FC = () => {
  ...
  const grouped = filtered.reduce<Record<CollectionCategory, CollectionMetadata[]>>(
    ...
    { flow_player: [], media: [], system: [], custom: [] }
  );
  ...
  return (
    <div className="w-80 bg-slate-950/90 border-l border-slate-800/80 flex flex-col h-full select-none">
      ...
      <div className="p-4 border-b border-slate-800/80 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-white text-xs">קולקציות הפרויקט</h3>
            <p className="text-[10px] text-slate-400">Firestore Realtime API</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] bg-slate-900 hover:bg-slate-800 text-indigo-300 border border-indigo-500/30 px-2 py-1 rounded-lg transition"
      ...
      {/* Quick Add Custom Collection Box */}
      {showAddInput && (
        <form onSubmit={handleAddSubmit} className="p-3 bg-slate-900 border-b border-slate-800 space-y-2 animate-in fade-in">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              type="submit"
              className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs px-3 py-1 rounded-lg font-semibold transition"
            >
              פתח
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        {(['flow_player', 'media', 'system', 'custom'] as CollectionCategory[]).map((categoryKey) => {
